...
export class MinioObjectStorageService {
  ...
  async putObject(storageKey: string, data: Buffer, contentType: string): Promise<void> {
    await this.request({
      method: 'PUT',
      storageKey,
      body: data,
  ...
  private async request(params: {
    ...
    const canonicalUri = `/${this.getBucket()}/${this.encodePath(params.storageKey)}`;
    const payloadHash =
      params.method === 'PUT' && params.body
    ...
    if (params.method === 'PUT' && params.body) {
    ...
    if (!response.ok) {
      ...
      throw new ServiceUnavailableException(
        `MinIO object ${params.method} failed (${response.status}): ${text}`,
      );
    }
